...
class Point {
	constructor(x, y) {
		...
		this.radius = Utl.random(20,80); //Radio de los círculos
		let color_id = Math.floor(Utl.random(0,colorTheme.length));	
		this.color = colorTheme[color_id];

		let text_id = Math.floor(Utl.random(0,textTheme.length)); // Texto
		//habrá que hacer aquí para que no repita los textos
		this.text = textTheme[text_id]; //IGOR
	...
	draw() {	
		...
		//textos
		ctx.fillText(this.text,this.pos.x,this.pos.y,400,100);
		ctx.textAlign="center";
		ctx.textBaseline="alphabetic";
		ctx.font="bold 50px arial"; //estilo de texto
		ctx.fillStyle=this.color;

		/* //se vuelve alfa el color sólido
		ctx.globalCompositeOperation = 'xor'; */
		
		//se dibuja cuando se clica o se hace mouseover sobre un círculo
  	...
  	update() {
		...
		//Se juntan al empezar
		if(!this.linked){
			let angle = Utl.angleFrom(this.pos,this.target) //ángulo random
			this.pos.x += Math.cos(angle) * 0.3; //velocidad en x
			this.pos.y += Math.sin(angle) * 0.3; //velocidad en y
		}

		//Si no pinchamos sobre una bola o mantenemos activo
...
class Stick{
	constructor(point_array,stick_array,p0,p1,min_distance){
		...
		this.length = Math.max(min_distance,distance) + Utl.random(70,100); //distancia mínima y máxima coge random
		this.maxLength = this.length + 100;
		this.max_link_width = Math.min(this.p0.radius/3,this.p1.radius/3);  //grosor de las líneas dividido entre 3
	}
	update(){
	...
	draw(){
		...
		ctx.lineCap = "round"; //redondeamos el final de la línea
		ctx.beginPath();
...
class Synapse{
	constructor(){
		this.points = [];
		this.sticks = [];
		this.number_point = 10; //número de círculos
...
colorTheme = ["#3fc778","#00a383","#007dc7","#bcb7c5","#ea71bd","#e1534a","#ad3e50","#f29546","#e33247","#007dc7","#ea71bd","#64468d"];
...
textTheme = ["web","fotografía","diseño gráfico","Arte","desarrollo","rrss","seo","sem","logo"]; // igor
