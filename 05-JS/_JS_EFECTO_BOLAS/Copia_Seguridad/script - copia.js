...
class Point {
  constructor(x, y) {
    ...
    this.radius = Utl.random(20,40); //20 o 40
    let color_id = Math.floor(Utl.random(0,colorTheme.length));	
    this.color = colorTheme[color_id];

	let text_id = Math.floor(Utl.random(0,textTheme.length)); // Igor
...
class Synapse{
	constructor(){
		this.points = [];
		this.sticks = [];
		this.number_point = 15; //número de círculos
...
//link_color = "#280f19";
 ...
 textTheme = ["WEB","FOTO","DISEÑO"]; // igor
